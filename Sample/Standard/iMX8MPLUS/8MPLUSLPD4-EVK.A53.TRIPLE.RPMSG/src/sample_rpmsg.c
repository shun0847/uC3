...
static int rpmsg_endpoint_cb2(struct rpmsg_endpoint *ept, void *data, size_t len, uint32_t src, void *priv)
{
    /* service 2 */
    (void)priv;
    (void)src;
...
static void rpmsg_service_unbind2(struct rpmsg_endpoint *ept)
{
    (void)ept;
    /* service 2 */  
    rpmsg_destroy_ept(&rp_ept[2]);
    memset(&rp_ept[2], 0x0, sizeof(struct rpmsg_endpoint));
    evt_svc_unbind[2] = 1;
    return ;
}
...
void SheardMemoryTask(VP_INT exinf)
{
    ...
    if (svcno == 0UL) {
        ...
        memset((void*)(CFG_SHMEM_CTL_BASE2), 0, CFG_SHMEM_CTL_SIZE2);
...
int app(struct rpmsg_device *rdev, void *platform, unsigned long svcno)
{
    ...
    if (svcno == 0UL) {
        ...
        ret = rpmsg_create_ept(&rp_ept[2], rdev, CFG_RPMSG_SVC_NAME2,
                       APP_EPT_ADDR, RPMSG_ADDR_ANY,
                       rpmsg_endpoint_cb2,
                       rpmsg_service_unbind2);
    }
    if (ret) {
...
T_CTSK const ctsk_shmem2 = {(TA_HLNG|TA_ACT|TA_FPU), (VP_INT)2U, (FP)SheardMemoryTask, 3U, 0x1000U, 0, "SheardMemoryTask"};
...
T_CTSK const ctsk_main2 = {(TA_HLNG|TA_ACT|TA_FPU), (VP_INT)2U, (FP)RPMsgTask, 3U, 0x1000U, 0, "RPMsgTask"};
...
void sample_rpmsg(void)
{
    ...
    if (id == ID_CORE3) {
        ...
        /* Core1 */
        (void)acre_tsk((T_CTSK*)&ctsk_shmem2);
        (void)acre_tsk((T_CTSK*)&ctsk_main2);
    }
