...
static void Core3Task(VP_INT exinf);
static void initpr(void);

/**
 * Core3Task
 */
void Core3Task(VP_INT exinf)
{
    while (1) {
        (void)vset_flg((ID)ID_CORE3, Core3FlagID, 0x0001U);
        // set_flg(Core3FlagID, 0x0001U) is also available because flag setting is done on this CPU core

        (void)dly_tsk(1100U);
        (void)clr_flg(Core3FlagID, ~0x0001U);

        (void)vclr_flg((ID)ID_CORE3, Core3FlagID, ~0x0001U);
        // vclr_flg(Core3FlagID, ~0x0001U) is also available because flag setting is done on this CPU core
 ...
 */
static const T_CTSK ctsk_core3 = {TA_HLNG | TA_ACT | TA_FPU, (VP_INT)0, (FP)Core3Task, 8, 0x800, 0, "Core3Task"};
...
static void initpr(void)
{
    ...
    Core3TaskID = acre_tsk((T_CTSK *)&ctsk_core3);
    Core3FlagID = acre_flg((T_CFLG *)&cflg);
}
