...
static VB const *const  line_msg = "--------------------------------\r\n";
static VB const *const  core1_msg1 = "Core1 task is active...\r\n";
static VB const *const  core1_msg2 = "Core1 task is sleeping...\r\n";
static VB const *const  core2_msg1 = "Core2 task is active...\r\n";
static VB const *const  core2_msg2 = "Core2 task is sleeping...\r\n";
static VB const *const  core3_msg1 = "Core3 task is active...\r\n";
static VB const *const  core3_msg2 = "Core3 task is sleeping...\r\n";
...
static void uart_send_task(VP_INT exinf)
{
    ...
    while (1) {
        ...
        // Core #2
        ercd = vpol_flg(ID_CORE2, Core2FlagID, 0x0001U, TWF_ORW, &flgptn);
        if (ercd == E_OK) {
            txcnt = strlen(core2_msg1);
            (void)puts_com(ID_UART, (VB *)core2_msg1, &txcnt, TMO_FEVR);
        } else {
            txcnt = strlen(core2_msg2);
            (void)puts_com(ID_UART, (VB *)core2_msg2, &txcnt, TMO_FEVR);
        }
        // Core #3
        ercd = vpol_flg(ID_CORE3, Core3FlagID, 0x0001U, TWF_ORW, &flgptn);
        if (ercd == E_OK) {
            txcnt = strlen(core3_msg1);
            (void)puts_com(ID_UART, (VB *)core3_msg1, &txcnt, TMO_FEVR);
        } else {
            txcnt = strlen(core3_msg2);
            (void)puts_com(ID_UART, (VB *)core3_msg2, &txcnt, TMO_FEVR);
        }

        txcnt = strlen(line_msg);
        (void)puts_com(ID_UART, (VB *)line_msg, &txcnt, TMO_FEVR);
    }
...
void WupTask(VP_INT exinf)
{
    ...
    while (1) {
        ...
        ercd = vpol_flg(ID_CORE2, Core2FlagID, 0x0001U, TWF_ORW, &flgptn);
        if (ercd == E_TMOUT) {
            (void)vwup_tsk((ID)ID_CORE2, Core1TaskID);
        }
        ercd = vpol_flg(ID_CORE3, Core3FlagID, 0x0001U, TWF_ORW, &flgptn);
        if (ercd == E_TMOUT) {
            (void)vwup_tsk((ID)ID_CORE3, Core1TaskID);
        }
    }
}
