...
static void Core2Task(VP_INT exinf);
static void initpr(void);

/**
 * Core2Task
 */
void Core2Task(VP_INT exinf)
{
    while (1) {
        (void)vset_flg((ID)ID_CORE2, Core2FlagID, 0x0001U);
        // set_flg(Core2FlagID, 0x0001U) is also available because flag setting is done on this CPU core

        (void)dly_tsk(700U);
        (void)clr_flg(Core2FlagID, ~0x0001U);

        (void)vclr_flg((ID)ID_CORE2, Core2FlagID, ~0x0001U);
        // vclr_flg(Core2FlagID, ~0x0001U) is also available because flag setting is done on this CPU core
 ...
 */
static const T_CTSK ctsk_core2 = {TA_HLNG | TA_ACT | TA_FPU, (VP_INT)0, (FP)Core2Task, 8, 0x800, 0, "Core2Task"};
...
static void initpr(void)
{
    ...
    Core2TaskID = acre_tsk((T_CTSK *)&ctsk_core2);
    Core2FlagID = acre_flg((T_CFLG *)&cflg);
}
