...
static ER _kernel_stpcyc(T_CYC *cyc);
static BOOL _kernel_stpcyc_1(T_SSB *par, BOOL retcd);
...
static BOOL _kernel_stpcyc_1(T_SSB *par, BOOL retcd)
{
    par->p1 = (VP_INT)_kernel_stpcyc((T_CYC *)par->p1);
    return retcd;
}
...
ER stp_cyc(ID cycid)
{
    ...
    if (_kernel_systbl.icnt != 0U) {
        ...
        if (cyc == 0) {
            ...
            tcb->p1 = (VP_INT)cyc;
            tcb->sysfunc = (FP)&_kernel_stpcyc_1;
