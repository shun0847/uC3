...
ER _kernel_vacttsk_3(T_CMDBUF *buffer)
{
    ER ercd;
...
void _kernel_vacttsk_2(T_CMDBUF *buffer)
{
    ER ercd;

    ercd = _kernel_vacttsk_3(buffer);
...
void _kernel_vacttsk_1(T_SYNCTBL *syncbuf)
{
    syncbuf->error = _kernel_vacttsk_3(&syncbuf->cmdbuf);
    _kernel_synch_cache();
    syncbuf->fin = 1U;
    _kernel_synch_cache();
}

ER _kernel_vacttsk_h(ID coreid, ID tskid, UW seqno, FP ret_pc)
{
    T_TCB *tcb;
    T_SSB *cssb;
    XPARAM para[3];
    ...
    if ((ID)get_hcid() == coreid) {
        ...
        if (_kernel_systbl.icnt != 0U) {
            ...
            ercd = _kernel_async_sys((FN)TFN_VACT_TSK, (ID)_kernel_systbl.ctxid_s, seqno, (XPTR)((ADDR)ret_pc), XC_FUNC_IDX_VACTTSK_2, para);
            ...
            if (_kernel_xcore_ext.sync_syscall_mode == SYNCCALL_ASYNC_MODE) {
                ...
                tcb->p1 = (VP_INT)TFN_VACT_TSK;
                tcb->p2 = (VP_INT)_kernel_systbl.ctxid_s;
                tcb->p3 = (VP_INT)seqno;
                tcb->p4 = (VP_INT)ret_pc;
                tcb->p5 = (VP_INT)XC_FUNC_IDX_VACTTSK_1;
                tcb->p6 = (VP_INT)para;
            ...
            if (cssb->stat.catr == TA_INI) {
                ...
                if (_kernel_xcore_ext.sync_syscall_mode == SYNCCALL_ASYNC_MODE) {
                    ...
                    ercd = _kernel_sync_sys((FN)TFN_VACT_TSK, (ID)_kernel_systbl.ctxid_s, seqno, (XPTR)((ADDR)ret_pc), XC_FUNC_IDX_VACTTSK_1, para);
                }
            }
        }
    }
...
ER vact_tsk(ID coreid, ID tskid)
{
    return _kernel_vacttsk_h(coreid, tskid, 0U, 0);
}
