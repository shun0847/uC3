	      ...
	      memory_order order, int width)
{
	...
	if (io->ops.read)
		...
		return atomic_load_explicit((unsigned char *)ptr, order);
	else if (ptr && sizeof(atomic_ushort) == width)
		return atomic_load_explicit((unsigned short *)ptr, order);
	else if (ptr && sizeof(atomic_uint) == width)
		return atomic_load_explicit((unsigned int *)ptr, order);
	else if (ptr && sizeof(atomic_ulong) == width)
		return atomic_load_explicit((unsigned long *)ptr, order);
#ifndef NO_ATOMIC_64_SUPPORT
	else if (ptr && sizeof(atomic_ullong) == width)
		return atomic_load_explicit((unsigned long long *)ptr, order);
#endif
#endif
	metal_assert(0);
...
#include <metal/system/uc3/io.h>
#else
#include <metal/system/uc3mc2_solid/io.h>
#endif
