...
ER apl_ssl_ini(void)
{
    T_SSL_RAW_CERT ca, cert;
    T_SSL_RAW_KEY  cert_key;
    T_SSL_INI_CFG  cfg;

    /* SSL */
    ca.dat = apl_ssl_ca_cer;
#ifdef SSL_CER_PEM
    ca.len = strlen(apl_ssl_ca_cer);
    ca.type = CERT_TYPE_PEM;
#else
    ca.len = sizeof(apl_ssl_ca_cer);
    ca.type = CERT_TYPE_DER;
#endif

    cert.dat = apl_ssl_cer;
#ifdef SSL_CER_PEM
    cert.len = strlen(apl_ssl_cer);
    cert.type = CERT_TYPE_PEM;
#else
    cert.len = sizeof(apl_ssl_cer);
    cert.type = CERT_TYPE_DER;
#endif

    cert_key.dat = apl_ssl_cer_key;
#ifdef SSL_CER_PEM
    cert_key.len = strlen(apl_ssl_cer_key);
    cert_key.type = CERT_TYPE_PEM;
#else
    cert_key.len = sizeof(apl_ssl_cer_key);
    cert_key.type = CERT_TYPE_DER;
#endif

    cfg.ca = &ca;
    cfg.ca_num = 1;
    cfg.host_cert = &cert;
    cfg.host_key = &cert_key;
	cfg.client_authentication = SSL_CLI_CERT_NOT_REQ; /* SSL_CLI_CERT_MUST_REQ or SSL_CLI_CERT_MAY_REQ or SSL_CLI_CERT_NOT_REQ */
...
#include "ssl_cph.h"
#include "ssl_arc4.h"
extern T_SSL_CFG_USR_VAL gSSL_CFG_USR_VAL;

const VB* get_proto2str(INT ver)
{
	switch (ver) {
	case SSL_MINOR_V0:	return "SSLv3.0";
	case SSL_MINOR_V1:	return "TLSv1.0";
	case SSL_MINOR_V2:	return "TLSv1.1";
	case SSL_MINOR_V3:	return "TLSv1.2";
	default:			return "Unknown";
	}
}

ER shell_usr_cmd_sslconf(VP ctrl, INT argc, VB *argv[])
{
	T_SSL_CFG_USR_VAL const *cv = &gSSL_CFG_USR_VAL;
	
    shell_printf(ctrl, "\r\n-- SSL library infomation --\r\n");
	shell_printf(ctrl, "  TLS server support      : %s \r\n", 
		(is_tls_server_sup()) ? "Yes" : "No");
	
    shell_printf(ctrl, "\r\n-- SSL user configuration --\r\n");
	shell_printf(ctrl, "  CFG_SSL_VERSION         = %s(%d) \r\n", get_proto2str(cv->proto_ver), cv->proto_ver);
	shell_printf(ctrl, "  CFG_SSL_VERSION_MIN     = %s(%d) \r\n", get_proto2str(cv->proto_ver_min), cv->proto_ver_min);	
	shell_printf(ctrl, "  CFG_SSL_CERT_SIZE       = %d \r\n", cv->cert_size);
	shell_printf(ctrl, "  CFG_SSL_CERT_DEPTH      = %d \r\n", cv->cert_depth);
	shell_printf(ctrl, "  CFG_SSL_MAX_SESSION     = %d \r\n", cv->max_session);
	shell_printf(ctrl, "  CFG_SSL_MAX_CONNECTION  = %d \r\n", cv->max_connect);
	shell_printf(ctrl, "  CFG_SSL_CA_CERT_MAX     = %d \r\n", cv->max_ca_cert);
	shell_printf(ctrl, "  CFG_SSL_CRYP_TMO        = %d \r\n", cv->tmo_cryp);
	shell_printf(ctrl, "  CFG_SSL_HASH_TMO        = %d \r\n", cv->tmo_hash);
	shell_printf(ctrl, "  CFG_SSL_KEY_SIZE        = %d \r\n", cv->key_size);
	shell_printf(ctrl, "  SSL_NET_BUF_MPF_SZ      = %d \r\n", cv->ssl_buf_len);
